...
def get_statistical_significance():
    ...
    for path in sorted(LOCAL_DISTANCES_EUCLIDEAN):
        ...
        # For every video_sequence we have to compare two populations of local distances in order
        ...
        # landmark_significativi list. Since we don't know who these populations are, we have
        ...
        video_sequence = pd.read_csv(path, header=None)
        splits = list(range(2, len(video_sequence) - 1))

        # These structure keeps track of the statistical relevant landmarks for every split tried.
        landmarks_per_split = {}

        for split in splits:
            landmark_significativi = []
            df_1 = video_sequence.iloc[:split, :] # Population 1
            df_2 = video_sequence.iloc[split:, :] # Population 2
            ...
            for i in range(0, 467):
                bool, significant = pvalue_test(vec1=df_1[i], vec2=df_2[i], alpha_value=ALPHA)
                if bool:
                    landmark_significativi.append(i)
            landmarks_per_split[split] = landmark_significativi
        ...
        max_value = 0; split = 0; key_list = list(landmarks_per_split); size_list = 0
        ...
        for i in range(1, size_list):
            key1 = key_list[i - 1]
            key2 = key_list[i]
            diff = (len(landmarks_per_split[key2]) - len(landmarks_per_split[key1])) # computing the Delta
            if diff > max_value:
                max_value = diff
                split = key2
        if max_value == 0 and split == 0:
            subjects_split_history[path[16:24]] = [[1, 2, 3], landmarks_per_split[2]]
        else:
            ...
            subjects_split_history[path[16:24]] = [[split - 1, split, split + 1], landmarks_per_split[split]]
